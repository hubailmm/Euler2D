...
public:
	...
	static inline void executeStepFnc(int executeFnc,PointStruct2D *point)
	{
		switch(executeFnc)
		{
			...
			case(1):
			{
				...
				rhs_w[1] += 0.1 * c0 * point->C_input[1 + inputShift] * point->C_constants[0];	// changed			
				rhs_w[2] += 0.1 * c0 * point->C_input[2 + inputShift] * point->C_constants[0];  //changed
	...
	static void init(int i,int j,InitPoint2D *point)
	{
		...
		point->U_constants[0] = exp(-pow((x*x + (y-.25*dy)*(y-.25*dy))/1,8));
